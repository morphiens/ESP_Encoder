...
// ─── Pin Definitions
// ────────────────────────────────────────────────────────── GPIO numbers match
// the user's pin table exactly.
#define PIN_SEL D7  // SSI_SPI_SEL : HIGH = SSI, LOW = SPI
#define PIN_CLK D8  // SCL / CLK   : shared clock
#define PIN_NSL D10 // NSL / DIN   : SSI enable / SPI MOSI
#define PIN_DO D9   // DO  / DOUT  : data from encoder / SPI MISO

// ─── SSI Settings
// ───────────────────────────────────────────────────────────── Bit-banged;
// half-period determines SSI clock speed. 5 µs half-period → ~100 kHz SSI clock
// (datasheet max ~1 MHz)
...
// ─── SPI Settings
// ───────────────────────────────────────────────────────────── SPI_MODE3:
// CPOL=1 (idle high), CPHA=1 (sample on 2nd / rising edge)
...
void printPosition(const SSIFrame &f) {
  ...
  Serial.printf("[SSI] Pos=%5u  Angle=%8.4f deg  Magnet=%s%s%s\n", f.position,
                deg, magnetStatus(f.mhi, f.mlo), f.mhi ? " (MHi)" : "",
                f.mlo ? " (MLo)" : "");
}
...
void printHelp() {
  ...
  Serial.println(
      F("║  c       — Toggle continuous position stream          ║"));
  Serial.println(
      F("║  s       — Dump all SPI config registers              ║"));
...
void handleSerial() {
  ...
  case 'u':
    unlockRegisters();
    Serial.println(
        F("[SPI] Config registers unlocked (0xAB written to Lock reg)"));
    break;
  ...
  case 'b': {
    ...
    if (!line.equalsIgnoreCase("burn")) {
      ...
      Serial.println(
          F("[WARN]    Type exactly 'burn' and press Enter to confirm."));
      break;
    }
...
void setup() {
  ...
  // Hardware SPI init (pins will be re-claimed by SPI.beginTransaction when
  // needed)
